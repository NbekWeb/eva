...
<template>
  ...
  >
    ...
    <div class="mx-32 mt-6 max-2xl:mt-5  max-lg:mx-10 max-sm:mx-0">
        <h2 class="text-dark-100 text-5xl text-center font-semibold mb-12 max-2xl:mb-8 max-xl:mb-6 max-sm:mb-5">
            {{ blog.title }}
        </h2>
        <img :src="blog.images?.[0]?.image" class="w-full h-auto"/>
        <div v-html="blog.description" class="mt-10 max-2xl:mt-6 max-xl:mt-3"></div>
    </div>
  </div>
</template>
